...
export function checkPayloadSize(req: FnRequest, res: FnResponse): boolean {
  const contentLength = parseInt((req.headers["content-length"] as string) ?? "0", 10);
  if (contentLength > MAX_PAYLOAD_BYTES) {
    res.status(413).json(err("PAYLOAD_TOO_LARGE", `Payload excede o limite de ${MAX_PAYLOAD_BYTES / 1024} KB.`));
    return false;
  }
