...
import { shadowEligibleForPhone } from "./shadow_config";
...
export async function runShadowObservation(input: ShadowObservationInput): Promise<ShadowObservationOutcome> {
  try {
    const eligible = await shadowEligibleForPhone(input.channelPhone);
    if (!eligible) return { ran: false, reason: "NOT_ELIGIBLE_FLAG_OR_ALLOWLIST" };
    ...
    console.error("[shadow_runner] falha (não bloqueia webhook real):", (e as Error).message);
