/**
 * _audit_orc000036_os17_readonly_2026-08-16.js
 * READ-ONLY — mapeia todos os documentos ligados ao fixture
 * E2E_GOLIVE_SMOKE2_20260816 (Orçamento #000036 / OS #17) antes de escrever
 * o script de limpeza definitivo. Nada é gravado.
 *
 * Uso: node scripts/_audit_orc000036_os17_readonly_2026-08-16.js
 */
'use strict';
...
(async () => {
  ...
  const orc = (orcamentos || []).find(o => (o.cliente || '').indexOf('E2E_GOLIVE_SMOKE2_20260816') !== -1 || o.num === '000036' || o.numero === '000036');
  console.log('\n=== ORCAMENTO ===');
  ...
  const txMatches = (finTx || []).filter(t => String(t.os) === '17' || String(t.osId) === osId);
  console.log('\n=== FIN_TX matches (' + txMatches.length + ') ===');
  console.log(JSON.stringify(txMatches, null, 2));

  const leadEntries = Object.entries(crmLeads || {}).filter(([k, v]) => (v && v.orcamentoId === (orc && orc.id)));
  console.log('\n=== CRM_LEADS matches ===');
  console.log(JSON.stringify(leadEntries, null, 2));

  const cliente = (clientes || []).find(c => (c.nome || '').indexOf('E2E_GOLIVE_SMOKE2_20260816') !== -1);
  console.log('\n=== CLIENTE ===');
  console.log(JSON.stringify(cliente, null, 2));

  const stockLogMatches = (stockLog || []).filter(l => l.osId === osId);
  console.log('\n=== ERP_STOCK_LOG matches (osId=' + osId + ') ===');
